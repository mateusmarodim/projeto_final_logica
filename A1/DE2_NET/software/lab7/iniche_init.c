...
#include "../lab7_bsp/HAL/inc/includes.h"

#include "../lab7_bsp/system.h"
...
#include "../lab7_bsp/iniche/src/h/nios2/ipport.h"
#include "../lab7_bsp/iniche/src/h/tcpport.h"
#include "../lab7_bsp/iniche/src/h/libport.h"
#include "../lab7_bsp/iniche/src/nios2/osport.h"
#include "basic_io.h"
#include "LCD.h"
	...
	*/
	...
	while(1){
		...
		if (recv(SocketFD, buf, sizeof(buf), 0) < 0) {
			...
			if (strcmp(buf, blank)) {// imprime só se não for mensagem vazia
				printf("Msg recebida: %s\n", buf);
				memcpy(cmd, buf, 16 * sizeof(char));

				int numero = atoi(cmd);
				IOWR(0, 0, numero);
				int numero_negado = IORD(0, 0);

				printf("Numero: %d\nComplemento de 1: %d\n", numero, numero_negado);

//				char str[32];
//				sprintf(str, "%d", numero_negado);
